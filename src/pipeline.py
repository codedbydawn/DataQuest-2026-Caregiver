...
# =============================================================================
# SECTION 10 — K-Means Clustering on SHAP Profiles
# =============================================================================
# Goal: group caregivers by WHAT drives their burnout risk (not by risk level).
# Two caregivers with the same burnout score can land in different clusters if
# different features are responsible for that score.
#
# Why cluster on SHAP values instead of raw features?
#   Raw features mix signal and noise.  SHAP values encode each feature's
#   actual influence on burnout prediction, so clusters are directly
#   interpretable in terms of policy-relevant risk drivers.
#
# k selection:
#   • Elbow curve (inertia) — suggests where adding clusters has diminishing
#     returns on variance explained.
#   • Silhouette score     — measures how well-separated clusters are.
#     We pick the k with the highest silhouette score automatically.
#
# StandardScaler is applied before clustering so that features with large
# SHAP magnitudes don't dominate the distance metric.

from sklearn.cluster import KMeans
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import silhouette_score

K_RANGE = range(2, 9)   # test k = 2 through 8

scaler      = StandardScaler()
shap_scaled = scaler.fit_transform(shap_df)   # shape: (n_samples, n_features)
...
for k in K_RANGE:
    ...
    # sample_size speeds up silhouette calculation on large datasets
    silhouettes.append(silhouette_score(shap_scaled, labels, sample_size=2000, random_state=42))
...
# Attach cluster labels back to feature and SHAP dataframes
X_clustered            = X.copy()
X_clustered['cluster'] = cluster_labels
shap_df['cluster']     = cluster_labels
y_clustered            = y_clf.copy()
y_clustered.name       = 'burnout_high'

print(f'\nCluster sizes:\n{pd.Series(cluster_labels).value_counts().sort_index()}')


# =============================================================================
# SECTION 11 — Per-Cluster Reporting
# =============================================================================
# For each cluster, this section prints:
#   • Size and % of the full sample
#   • High-burnout rate (% of respondents labelled burnout_high = 1)
#   • Top N risk drivers by mean |SHAP|, with direction (▲ = increases risk,
#     ▼ = decreases risk)
#   • Demographic/contextual snapshot (column medians)
#   • A bar chart of top risk drivers for easy copy-paste into reports
#
# This output is intended for government/nonprofit stakeholders to decide
# WHERE to allocate resources and WHICH risk factors to address per group.
#
# Adjust TOP_N_FEATURES and DEMO_COLS as needed.

TOP_N_FEATURES = 10   # number of top SHAP drivers to show per cluster

# Demographic columns to include in the per-cluster snapshot
# Change or extend this list to include variables most relevant to your audience
DEMO_COLS = ['SEX', 'MARSTAT', 'PRV', 'FAMINCG1', 'VISMIN',
             'BPR_16', 'LAN_01', 'SENFLAG', 'COW_10', 'UWS230GR']
DEMO_COLS_AVAIL = [c for c in DEMO_COLS if c in X_clustered.columns]

# Exclude the cluster column when computing per-cluster SHAP means
shap_feat_cols = [c for c in shap_df.columns if c != 'cluster']

n_total = len(cluster_labels)
print('=' * 65)
print('  CAREGIVER BURNOUT RISK — CLUSTER PROFILES')
print('  (clusters defined by shared risk-driver patterns, not burnout level)')
print('=' * 65)

for c in sorted(pd.Series(cluster_labels).unique()):
    mask      = X_clustered['cluster'] == c
    n_cluster = mask.sum()
    burnout_rt = y_clustered[mask].mean()

    # Top SHAP drivers for this cluster (by mean absolute SHAP value)
    cluster_shap = shap_df.loc[mask, shap_feat_cols]
    top_features = (
        cluster_shap.abs().mean()
        .sort_values(ascending=False)
        .head(TOP_N_FEATURES)
    )

    print(f'\n── Cluster {c} ── {n_cluster} respondents ({n_cluster / n_total * 100:.1f}%)  |  '
          f'High-burnout rate: {burnout_rt * 100:.1f}%')
    print(f'  Top {TOP_N_FEATURES} risk drivers (mean |SHAP|):')
    for feat, val in top_features.items():
        # Positive mean SHAP → feature pushes prediction toward HIGH burnout
        direction = '▲' if cluster_shap[feat].mean() > 0 else '▼'
        print(f'    {direction}  {feat:<25}  {val:.4f}')

    print(f'  Demographic snapshot (column medians):')
    demo_snap = X_clustered.loc[mask, DEMO_COLS_AVAIL].median().round(1)
    for col, val in demo_snap.items():
        print(f'      {col:<20}  {val}')
    ...
    ax.set_xlabel('Mean |SHAP value|')
    ax.set_title(f'Cluster {c} — Top Risk Drivers  '
                 f'(n={n_cluster}, burnout rate={burnout_rt * 100:.1f}%)')
    plt.tight_layout()
    plt.show()

print('\n' + '=' * 65)
print('  Interpretation guide:')
print('  ▲ = this feature pushes burnout risk HIGHER for this cluster')
print('  ▼ = this feature pushes burnout risk LOWER for this cluster')
print('  Mean |SHAP| = how strongly this feature influences the prediction')
print('=' * 65)
